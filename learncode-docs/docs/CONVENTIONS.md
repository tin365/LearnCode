...
Success responses return the resource directly (bare values, no envelope). For
example, `GET /problems/:id` returns a `Problem`, not `{ data: Problem }`.

Error responses return `{ error: <string or Zod issue object> }` with the
appropriate HTTP status code. Auth failures return `{ error: 'Invalid email or password' }`;
validation failures return `{ error: <Zod flatten output> }`.

**Why no envelope:** there is no public API, no pagination, no versioning, and
the frontend is the only consumer — the envelope would buy us nothing and add
a level of unwrapping at every call site. If a future endpoint genuinely needs
metadata (pagination cursor, totals), introduce it on that endpoint's response
type — don't pre-pay the cost everywhere.
